...
module.exports = {
    getAllComment:(req,res) =>{
        connection.query("SELECT * FROM events_comment",(error,result,field)=> {
            if (error){
                res.status(400).send({
                    error
                });
    ...
    addComment: (req,res) =>{
        ...
        connection.query(`SELECT * FROM events_comment where idevents LIKE N'%${prmkey}%' ORDER BY idevents DESC LIMIT 0,1`,
            (error, results, fields) => {
                if (error)  {
                    ...
                    if(results.length >= 1){
                        ...
                        connection.query(`INSERT INTO events_comment values('${idevents}','${req.body.namaevents}','${req.body.startdate}','${req.body.enddate}','${req.body.starttime}','${req.body.endtime}','${req.body.eventlocation}','${req.body.eoname}','${req.body.pic}','${req.body.picphone}','${req.body.eooffice}','NEEDAPPROVAL','')`,(error,result,field)=> {
                            if (error){
                                res.status(400).send({
                                    error
                                });
